...
if not all_snapshot_dates:
    st.warning("No position snapshots found.")
    st.page_link("pages/1_Upload.py", label="Upload a statement with positions", icon="📤")
    st.stop()
...
tab_summary, tab_beta, tab_details, tab_export, tab_recon = st.tabs(
    ["Summary", "Beta", "Asset Details", "Export", "Reconciliation"]
)
...
# ── Pre-compute consolidated data (used by Summary + Export tabs) ──────────
...
with tab_summary:
    ...
    csv_summary = consol_df[["symbol", "quantity", "breakeven", "market_value", "pct_of_account"]].copy()
    csv_summary.columns = ["Symbol", "Qty", "Breakeven", "Market Value", "% of Account"]
    st.download_button(
        label="Download Summary CSV",
        data=csv_summary.to_csv(index=False),
        file_name=f"holdings_summary_{selected_account}_{as_of.isoformat()}.csv",
        mime="text/csv",
    )

# ── Tab: Asset Details ────────────────────────────────────────────────────

# ── Helper: render asset class detail tables ──────────────────────────────


def _render_asset_details(container):
    """Render per-asset-class tables inside the given Streamlit container."""
    if has_market_data:
        display_cols = [
    ...
    for asset_class, group in df.groupby("asset_class", sort=True):
        ...
        container.subheader(f"{label} ({len(group)})")
        ...
        container.dataframe(detail_df, use_container_width=True, column_config=detail_col_config)


with tab_details:
    _render_asset_details(tab_details)

    # CSV export for detailed holdings
    all_detail_rows = df.copy()
    csv_cols = [c for c in ["symbol", "asset_class", "quantity", "cost_basis", "cost_value",
                            "market_price", "market_value", "unrealized_pnl",
                            "expiry", "strike", "right"] if c in all_detail_rows.columns]
    st.download_button(
        label="Download Details CSV",
        data=all_detail_rows[csv_cols].to_csv(index=False),
        file_name=f"holdings_detail_{selected_account}_{as_of.isoformat()}.csv",
        mime="text/csv",
    )
